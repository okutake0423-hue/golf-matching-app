...
import type { MahjongScheduleDoc } from '@/types/mahjong-schedule';
...
export function MahjongScheduleCalendar({
  value,
  onChange,
  schedules,
  activeMonthKey,
    ...
    list.forEach((s) => {
      if (s?.dateStr) {
        ...
        if (s.type === 'RECRUIT') {
          ...
          if (s.isCompetition) current.hasCompetition = true;
        } else if (s.type === 'WISH') {
  ...
  const tileContent = useCallback(
    ({ date }: { date: Date }) => {
      ...
      if (!info) return null;
      if (info.hasCompetition) return <span className={styles.dotCompetition} aria-hidden />;
      if (info.hasWish) return <span className={styles.dotWish} aria-hidden />;
      if (info.hasRecruit) return <span className={styles.dot} aria-hidden />;
      return null;
    },
    [dateStrMap]
  );
  ...
  const activeStartDate = activeMonthKey ? toMonthStart(activeMonthKey) ?? undefined : undefined;
  ...
  const handleActiveStartDateChange = useCallback(
    (args: { activeStartDate: Date | null }) => {
      ...
      onActiveMonthChange(`${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`);
    },
    [onActiveMonthChange]
  );
    ...
    <div className={styles.wrapper}>
      <Calendar
        ...
        formatShortWeekday={(_, date) =>
          ['日', '月', '火', '水', '木', '金', '土'][date && typeof date.getDay === 'function' ? date.getDay() : 0]
        }
